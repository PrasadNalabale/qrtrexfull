...
def renewMembership(request):
    ...
    return render(request,'qr/renewMembership.html',{'restaurant':restaurant,'membership_choices': UserMembership.MEMBERSHIP,})
    ...
    if request.method == "POST":
        ...
        # Multiple QR codes
            ...
            if not total_tables.isdigit():
                messages.error(request, "Total tables must be a valid number.")
            ...
            messages.error(request, "Please enter table number or total tables.")
            return redirect('createQr')
...
def category(request):
    ...
    if request.user.is_authenticated:
        if request.method=="POST":
            ...
            membership = UserMembership.objects.get(restaurant=restaurant)
            return render(request,'qr/partials/menu_categories.html',{'user':user,'menus':menuCategory,'restaurant':restaurant,'membership':membership})
    else:
        return redirect('login')
@login_required
...
def update_menu_item(request,id):
    ...
    return render(request,'qr/update_menu_item.html',context)
...
def allOffers(request):
    restaurant = Restaurant.objects.get(user=request.user)
    offers = Offer.objects.filter(restaurant=restaurant)
    return render(request,'qr/partials/allOffers.html', {'offers':offers,'restaurant':restaurant})
