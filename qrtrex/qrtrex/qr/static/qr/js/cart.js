...
function downloadReceipt() {
    ...
    const doc = new jsPDF();
    const cart = getCartItems(); // Assume this returns [{ name, price, qty }]
    const logoUrl = document.querySelector('#restaurant-logo')?.src || null;
    const name = document.getElementById('restaurant-name').textContent.trim();
    const address = document.getElementById('restaurant-address').textContent.trim();

    // Calculations
    let subtotal = cart.reduce((total, item) => total + item.price * item.qty, 0);
    let cgst = +(subtotal * 0.05).toFixed(2);
    let sgst = +(subtotal * 0.05).toFixed(2);
    let total = +(subtotal + cgst + sgst).toFixed(2);
    ...
    function drawContent() {
        ...
        y += 6;
        doc.text(`CGST (5%): ${cgst.toFixed(2)}`, 130, y);
        y += 6;
        doc.text(`SGST (5%): ${sgst.toFixed(2)}`, 130, y);
        y += 6;
        doc.setFontSize(13);
